...
Item::Item(QClipboard::Mode mode)
    : m_mode(mode),
      m_valid(true)
{
...
//    qDebug() << "Item::Item(QClipboard::Mode mode)" << mode << clipboard->text(mode) << mimeData->hasImage();
    ...
    {
        // any binary stuff
        m_contentType = Item::Binary;
    }
}

Item::Item(QClipboard::Mode mode, Item::ContentType contentType, const ClipboardContent &content)
    : m_mode(mode),
...
QClipboard::Mode Item::clipBoardMode() const
{
    return m_mode;
}
...
QString Item::displayRole() const
{
...
bool Item::operator==(const Item &other) const {
    ...
    return this->isValid() == other.isValid()
            && (this->clipBoardMode() == other.clipBoardMode()
                    || Preferences::Instance()->shouldSynchronizeClipboards()
                    )
            && this->content() == other.content();
}

QIcon Item::iconForContentType() const
{
    QString theme;
    switch (m_contentType)
    {
...
{
    dbg.nospace() << "Item(mode=" << c.clipBoardMode() << ", contentType=" << c.contentType() << ", text=" << c.content() << ")";
    return dbg.space();
}
